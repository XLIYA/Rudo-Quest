...
Hosted PostgreSQL uses certificate verification. Production state-changing requests require an allowed Origin, production request identity uses platform-forwarded address data, and Upstash-backed rate limiting fails closed when production credentials are absent. JSON and webhook bodies are streamed with byte limits. Production CSP uses request nonces, permits only exact version-pinned style payload hashes emitted by the installed UI dependencies, and does not allow unsafe inline scripts or eval.
...
Protected route SSR verifies JWT identity without adding PostgreSQL profile/notification reads to every page request. The client profile request is shared through the query cache, while profile text, avatars, unread counts, and route children are revealed only after a deterministic hydration snapshot. This removes the intermittent fast-response avatar/text hydration race without delaying data fetching. Vercel telemetry renders only on actual Vercel deployments, avoiding broken telemetry requests during local production smoke tests while retaining hosted monitoring. Development uses Turbopack to keep the complete authenticated route surface stable, while the production build remains Webpack-based for Serwist compatibility.
...
Serwist caches public shell assets and the offline route but uses `NetworkOnly` for application APIs and protected navigations. Its first `clientsClaim` no longer reloads a first-time visitor; a reload occurs only after the user activates a waiting application update. Selected reads are persisted in user-scoped IndexedDB, but cold restore requires a server-confirmed `/api/me`; a remembered user ID never unlocks private data offline. All mutations are blocked offline. Supabase Cron invokes the authorized POST endpoint every fifteen minutes, resolves the target and bearer credential from Supabase Vault, and keeps the Redis lock that prevents overlapping work. This preserves the required reminder interval on the linked Vercel Hobby project.
